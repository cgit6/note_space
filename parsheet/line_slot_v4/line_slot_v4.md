...
Base Game ==三顆以上 C1 的機率== *1 spin Free Game RTP* Free Game 平均 spin 次數。

其中 Free Game 平均 spin 次數利用等比級數公式計算
1 / (1- (FG 局數 *3 顆 FG C1 再觸發率) - (FG 局數* 4 顆 FG C1 再觸發率) - (FG 局數 * 5 顆 FG C1 再觸發率))
...
統計值:
CV = simple_std / mean
得分率 = bg_hit_rate + fg_trigger *fg_avg_spin* fg_hit_rate / (1+ fg_trigger * fg_avg_spin)
fg_trigger = FG 觸發率
